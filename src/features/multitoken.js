...
const url = "https://magenx404-server-production.up.railway.app/x404_auth/multitoken";
const path = new URL(url).pathname;

/**
 * x404-MultiToken: Multi-token verification
 * Verifies user holds multiple tokens (portfolio-based)
 */
export async function X404MultiToken(config) {
  const issued_jwt = localStorage.getItem("sjwt404_multitoken");
  ...
  const payload = buildSigningPayload(nonce, path, "MAGEN404_MULTITOKEN");
  ...
  const res = await fetch(url, {
    ...
    headers: {
      ...
      required_tokens: JSON.stringify(config.required_tokens || []),
      verification_mode: config.verification_mode || "ALL",
  ...
  if (res.status === 500 && data.status === "locerror") {
    ...
    return {
      ...
      message: data.message || "User does not meet multi-token requirements",
    };
    ...
    if (data.token) {
      localStorage.setItem("sjwt404_multitoken", data.token);
    }
    return {
      success: true,
