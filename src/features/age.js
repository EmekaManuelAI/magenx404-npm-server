...
/**
 * x404-Age: Wallet age verification
 * Verifies wallet has been active for minimum time
 */
export async function X404Age(config) {
  const issued_jwt = localStorage.getItem("sjwt404_age");
  ...
  console.log("Starting x404-Age authentication...");
  ...
  const payload = buildSigningPayload(nonce, path, "MAGEN404_AGE");
  ...
  const res = await fetch(url, {
    ...
    headers: {
      ...
      "X-404-Feature": "age",
      min_wallet_age_days: config.min_wallet_age_days.toString(),
      min_first_transaction_days: config.min_first_transaction_days.toString(),
  ...
  if (res.status === 500 && data.status === "locerror") {
    ...
    return {
      ...
      message: data.message || "Wallet does not meet age requirements",
    };
    ...
    if (data.token) {
      localStorage.setItem("sjwt404_age", data.token);
    }
    return {
      success: true,
